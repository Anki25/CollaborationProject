...
/* GET LIST OF ALL FORUM COMMENTS AS PER ID*/

fetchAllForumComments:function(id){
	return $http.get(BASE_URL+'/forumscommentlistperforum/'+id)
	.then(
			function(response){
				return response.data;
			},
			function(errResponse){
				console.error('Error while fetching ForumComments List');
				return $q.reject(errResponse);
			}				
);
},
...
fetchAllForumComment:function(){
	return $http.get(BASE_URL+'/forumscommentlist/')
	.then(
			function(response){
				return response.data;
			},
...
createComment:function(id,forumcomment){
	return $http.post(BASE_URL+'/forumcomment/'+id,forumcomment)
	.then(
			function(response){
				return response.data;
			},
